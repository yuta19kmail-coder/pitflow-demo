/* ========================================
   ...
   🔴 v1.151.0 洗車は**完TELを待たない**＝タスクボードにいる車も拾う。
     ...
     ④ コーティング・その他依頼 ＝1Y/3M・車販依頼バッジ＋coatingOK（受注OK）＝返車予定日も表示
     ⑤ 直近1か月のコーティング・その他予定 ＝1Y/3M・車販依頼バッジで 予約 or 入庫中（直近1か月）
...
function renderCarSales(){
  ...
  // ④ コーティング・その他依頼
  {
    ...
    h += _csSec('<i data-ic=sparkle data-ics=16></i> コーティング・その他依頼', '1Y/3M・車販依頼＋受注OK／返車予定日つき',
      s.open.map(c=>_csCard(c,'coating',_csRetLabel(c))).join(''),
      s.done.map(c=>_csDoneCard(c,'coating')).join(''));
  }
  // ⑤ 直近1か月のコーティング・その他予定（完了なし＝予定一覧）
  {
    h += _csSec('<i data-ic=calendar data-ics=16></i> 直近1か月のコーティング・その他予定', '予約・入庫中の 1Y/3M・車販依頼',
