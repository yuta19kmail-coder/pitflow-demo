...
/* ===================================================================
   ...
   ＝ 1枚目を保存したら、お客様と車が入ったまま2枚目の入力に入る。

   🔴 **1枚目もちゃんと刷る**（ゆうた指定「どっちも印刷はする」）。
      刷る条件は「印刷して保存」と同じ＝**関門を通った時だけ**（v1.78.0）。
      ＝紙が出たのに予約が無い、が起きない。
   🔴 **関門で止まったら、2枚目は開かない。**
      1枚目が保存できていないのに次の入力が始まると、打ちかけが行方不明になる。
   🔴 **保存そのものは「印刷して保存」の本物を呼ぶだけ**＝写しを作らない。
      ここに保存の手順を書き写すと、片方だけ直して食い違う。
   ⚠ 聞く時の言葉は「印刷して保存」のまま（黄色が空の時に
      「このまま印刷して保存しますか？」と出る）。長い名前をそのまま入れると文章が壊れる。
   =================================================================== */
function pitSaveAndPrintNext(){
  if (!_pitSaveOnce()) return;                        /* 🔴 v1.56.1 二度押しを飲み込む */
  pitSaveMenuClose();
  const c = state.cards.find(x => x.id === _editingCardId);
  const id = c ? c.id : _editingCardId;
  _pitCardGuard('印刷して保存', function(){
    _pitSaveAndPrintGo(c, id);                        /* 1枚目＝刷って保存して前の画面へ戻る（本物） */
    /* 2枚目＝お客様・車・作業タイプ・課・担当を持って、まっさらな新規予約を開く（views.js） */
    if (window.pitOpenNextReserveFrom) pitOpenNextReserveFrom(id);
  });
}
window.pitSaveAndPrintNext = pitSaveAndPrintNext;
...
/* ===================================================================
   ...
   ◎その他保存の中身
     承認に回して保存（v1.74.0）／ **印刷して保存して、この車でもう1件（v2.87.0）** ／
