...
(function () {
  ...
  /* ---- ①-0 タスクボードに戻す（完TELを取り消す）v1.137.0 -----------
     ...
     ◎なにをするか＝**`returnStage` を消すだけ。**
       ...
       ・売上の見込みは**1円も動かない** … `pitSalesTier` は「完TEL中」も「作業完了済」も同じ『**実績待**』
         （🔴 v1.167.0 で『確定』から切り出した。どちらも**作業は終わっている**という同じ意味）
  ...
  window.cvBackToReserve = function(){
    ...
    c.salesDate      = '';
    /* 🔴 v2.22.0 **実入庫日も消す。** 同じ理由＝入庫を取り消したのに「入庫した日」が残るのは嘘。
       ⚠ さらに v2.22.0 から `actualInAt` は「自動で未入庫に落とさない」印を兼ねている。
          ここで消さないと、**本当に入庫を取り消した車が二度と未入庫へ落ちなくなる**。
       ⚠ いつ入庫していたかはフローに残る（この下の logFlow）ので、記録は失われない。 */
    delete c.actualInAt;
  ...
  window.cvShConfirm = function(kind){
    ...
    /* 🔴🔴 v2.56.0 **記録の中身は pit-share.js の物差し1本（pitShakenApply）に寄せた。**
       ⚠ ここには v2.55.0 まで「済／再検」を組み立てる**写し**があった＝車検予定の画面と2か所。
          言い方を3つに増やす回に写しを残すと、片方だけ古くなるのが目に見えている
          （2026-08-15 の食い違い4件と同じ筋）。
       ⚠ この窓だけ**行った日を手で選べる**ので、先に「決定」をその日に置いてから物差しへ渡す
          （物差しは決定の日を実施日として使う）。 */
    ...
    _c.inspSchedule = _r.insp;
    if(_r.log && window.logFlow) logFlow(_c, _r.log);
