...
(function () {
  ...
  function upsertCustomerFromCard(c){
    if(!c) return;
    /* ================================================================
       🚙🔴 v2.78.0（ゆうた報告 2026-09-07）**代車・自社車両のカードは、顧客控えに1文字も書き戻さない。**
       ----------------------------------------------------------------
       ...
       ◎何が起きていたか
         代車の整備カードは、車名の欄に**代車の呼び名**（＝車両管理で入れた車種名。「タント茶」）を持つ。
         入庫すると、紐づけたお客様が**そのカードに写る**（お名前とお客様ID）。
         その後カードを開いて閉じる／保存すると、ここが走って
         **カードの車名を、そのお客様の車の「車種」に書き戻していた**＝呼び名で上書き。
         ⚠ 紐づけの設定そのものは無実。**カードの出口が犯人。**
       ◎決めごと（1行）
         🔴 **代車・自社車両のカードは、お客様の控えを作りも直しもしない。**
            ＝ あの車の持ち主は自社で、車名は車検証ではなく**社内の呼び名**だから、
              顧客控え（車検証の写し）に混ぜてよい種類の文字ではない。
       ⚠ 紐づけは今までどおり車両管理の「顧客車両との紐づけ」だけで作る（ここは読むだけ）。
       ⚠ 判定は `pitCardIntern`（js/intern-pit.js）1本。ここで `internKind` を直に見ない。
       ⚠ 中古・内部の社内車両も同じ扱いにした＝どれも**車検証の車種名ではない**のは同じで、
          しかも「自社代車」のような名前で**新しいお客様を作ってしまう**道でもあった。
       🔴 **すでに書き換わってしまった車種名は、これでは戻らない**（上書きした元の字が残っていないため）。
          直すのは顧客ビューの車の欄で人が手で。⚠ 探し方をまだ作っていない
          （紐づけた車の車種名が代車の呼び名と同じ＝怪しい、で機械的に出せる。データチェックに1項目・未着手）。
       ================================================================ */
