  ...
  window.HELP_CONTENTS = {
    ...
    course: `
      ...
      <ul>
        ...
        <li>🔴🔴 <strong>作業完了に入れると、点検担当・整備担当の窓が出ます（v1.174.0）。</strong>
            <ul>
              <li>チップの<strong>いちばん左に「なし」</strong>があります。
                  <strong>この車に該当者が居ない時は「なし」を押してください</strong>
                  （オイル交換なら点検担当は居ない／外注の板金なら整備担当は居ない、など）。</li>
              <li>🔴 <strong>空のままと「なし」はまったく別物です。</strong>
                  空のままは<strong>入れ忘れ</strong>として、データチェックが<strong>要対応</strong>で言い続けます。
                  「なし」を押しておけば、もう言いません。</li>
              <li>🔴 <strong>片方だけ空でも窓が出ます</strong>（整備だけ入れて点検を忘れる、がいちばん多いため）。
                  ドラッグでも ◀▶ でも、<strong>途中の段階を飛ばした時（クイック受注）でも</strong>出ます。</li>
              <li>止めはしません。「このまま進める」で今までどおり動きます。
                  ただし<strong>取り分（作業サマリー）に反映されない</strong>ので、その場で入れるのがいちばん早いです。</li>
            </ul></li>
    ...
    inspect: `
      <h2><i data-ic=search data-ics=16></i> データチェック</h2>
      <p>PitFlow の中のデータを全部読んで、<strong>おかしい所・入っていない所・止まっている車</strong>を並べる画面です。
         🔴 v1.170.0 で名前を<strong>「点検」から「データチェック」</strong>に変えました
         （PitFlow の「点検」は<strong>車の12ヶ月点検・タスクボードの点検待ち</strong>を指す言葉なので、取り違えないように）。</p>
      <p>いちばん上で<strong>2つを切り替えます。</strong></p>
      <ul>
        <li><strong>日常チェック</strong>＝PitFlow の中だけで分かる食い違いを、規則で拾います（お金はかかりません）。下の説明はこちらの話です。</li>
        <li><strong>クォーターチェック</strong>＝売上チェックリストPDFとの突合と、AIチェック。およそ週に1度まとめて見るためのものです（いまは準備中で、何が要るかを画面に出しています）。</li>
      </ul>
      <ul>
        <li>🔴🔴 <strong>この画面は何も隠しません。見つかったものは全部出ます。</strong>（返車が済んだ車のぶんも一緒に出ます）</li>
        <li>🔴🔴 <strong>ここに出ている数を 0 にするのが運用です（v1.172.0）。</strong>
            売上を確定させ、会社全体の履歴として残すデータなので、<strong>基本は全部直します。</strong><br>
            だから <strong>「これでOK（次から出さない）」と「この規則は出さない」は無くしました。</strong><br>
            ・<strong>「これでOK」</strong>で消えていた1件ずつのものは、<strong>全部出し直しています</strong>（数がいったん増えます）。<br>
            ・<strong>「この規則は出さない」</strong>を選んでいた規則は、<strong>規則そのものを消しました</strong>
            （2026-08-22・13本。＝うちのやり方に合っていない、と判断されたものなので、戻すのではなく無くしました）。<br>
            ⚠ うちのやり方に合っていない規則があれば、<strong>規則そのものを直します</strong>（画面から黙らせる道はありません）。ゆうたに言ってください。</li>
        <li><strong>「もう一度チェック」</strong>を押すと、その場でもう一度全部読み直します。上の<strong>時刻</strong>が変わるので、走ったことが分かります。変わりが無ければ「変わりはありません」と出ます。</li>
        <li><strong>重さは3つ。</strong>
            <strong>要対応</strong>＝放っておくとお金か信用が減るもの／
            <strong>確認</strong>＝たぶん入れ忘れ。見て決めるもの／
            <strong>気づき</strong>＝急ぎではないが、ここも 0 にするもの。<br>
        ...
        <li>🔴🔴 <strong>「ここを直す」（v1.170.0）</strong>＝指摘された<strong>その欄だけ</strong>を小さな窓で直せます。
            <strong>アーカイブまで行った車でも、この欄だけは誰でも直せます。</strong>
            窓に出てくるのは指摘された欄だけなので、<strong>ほかの欄はさわれません</strong>。<br>
            🔴 <strong>確定金額（請求額）と実績カウント日・確定返車日だけは、これまで通り管理者だけ</strong>です。
            管理でない人には<strong>見えますが「🔒 管理のみ」</strong>になります。<br>
            ⚠ 直すと、その車の<strong>フローと操作ログに「何を どこから どこへ」</strong>が必ず残ります。</li>
        <li>🔴🔴 <strong>規則は2種類あります（v1.173.0）。</strong><br>
            ・<strong>抜け・矛盾</strong>（40本）＝金額が空・日付が空・状態が食い違う。<strong>直すしかありません</strong>（閉じる道はありません）。<br>
            ・<strong>要判断</strong>（8本・むらさきの「見て決める」の印）＝<strong>概算だけの大口</strong>／見積と確定が大きく違う／
            けたが小さい／返車がずっと先／お休みの日の予定／代車を先に返す／同じ番号で名前が違う／課と担当の課が違う。
            <strong>データが間違っていないこともある</strong>ので、見て <strong>「確認した（合っている）」</strong>を押すと
            <strong>直す件数から外れます</strong>（＝0 にできます）。<br>
            🔴 <strong>「確認した」は要判断の7本にしか出ません。</strong>抜け・矛盾には出しません。<br>
            🔴 押しても<strong>消えません</strong>。下の「確認した（合っている）」にまとまり、<strong>誰がいつ決めたか</strong>が残ります。</li>
        <li><strong>「直した」</strong>の札は目印です（数は減りません）。本当に直っていれば<strong>次のチェックで自然に消えます</strong>。
            🔴 札は<strong>みんなで共有</strong>します（同じ車を2人で追いかけないため）。</li>
        <li>「返車予定日が入庫日より前」「実績の日が入庫日より前」は、
            <strong>本番を始める前に入庫した車では言いません</strong>
            （8月頭からのぶんを中旬にまとめて入れたので、打ち間違いではないため）。
            画面には出していません（裏で外しているだけです）。</li>
        <li><strong>書き出し</strong>＝結果を JSON で落とします。売上チェックリスト（整備ソフト）との突合や、AI にまとめて見てもらう時に使います。</li>
      </ul>
      <p>⚠ データチェックが「おかしい」と言うかどうかの物差しは、<strong>売上の区分・返車の日・代車の空き・車検の行ける日</strong>など
         <strong>ふだん画面が使っているものと同じ1本</strong>です。データチェックだけが別の答えを出すことはありません。</p>
    `,
