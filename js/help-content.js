  ...
  window.HELP_CONTENTS = {
    ...
    overview: `
      ...
      <ul>
        ...
        <li><strong>車検</strong>の「行ける日候補→決定→完了／再検／不合格」を見える化</li>
    ...
    newreserve: `
      ...
      <div class="help-step"><ol>
        ...
        <li><strong>作業タイプ</strong>（車検／12点／一般／オイル／B.P／1Y／3M／車販依頼）を選ぶと、概算の預かり日数・概算金額が自動で入ります（後で手直し可）。<br><strong>「車販依頼」</strong>＝車販部門に、<strong>コーティング以外の作業を頼む</strong>ときの印です（例：ルームクリーニング）。やってほしい中身は<strong>依頼事項に直接</strong>書いてください。押すと車販作業ビューの「コーティング・その他依頼」に出ます。</li>
        <li><strong>「その他」（v2.6.0）</strong>＝ふだんあまり使わない印の引き出しです。押すと開きます。
          <ul>
            <li><strong>付加</strong>（保証／保険／<strong>社員</strong>）… 作業タイプとセットで付ける印。売上・実績・完TELは<strong>通常どおり</strong>。社員は社割で金額が動くので、データチェックの金額チェックだけ外れます。</li>
            <li><strong>社内区分</strong>（<strong>中古</strong>＝自社の販売車両／<strong>代車</strong>＝自社の代車／<strong>内部</strong>＝それ以外の社内の車）… <strong>1つだけ</strong>選べます。選ぶと他のバッジは押せません（代車だけ 車検・12点・一般・B.P のどれか1つとセット）。</li>
            <li>社内区分を付けた車は、<strong>金額を聞かれません・完TEL・洗車・伝票・表紙の印刷がありません</strong>。概算金額／預かり日数／代車も入れません。完TEL済・完TEL依頼へドラッグすると「そのまま実績化します」の確認1枚だけで実績になります。</li>
            <li><strong>売上には数えません。</strong>実績カレンダーの<strong>「非カウント一覧」</strong>（実績ビューのボタンで切替）に残ります。売上ビューと作業サマリーには<strong>参考の台数だけ</strong>出ます。</li>
          </ul>
        </li>
    ...
    reserve: `
      ...
      <ul>
        ...
        <li>返車日と<strong>月がちがう</strong>と、その場で赤く出ます。データチェックにも出ます。
      ...
      <p>🃏 <strong>一覧は「カード」です（v2.1.0）。</strong>1件＝1枚。お客様の名前がいちばん大きく、
         <strong>整備ソフトと PitFlow が上下に並んで、日付も金額も同じ位置</strong>にあります。差はそのすぐ下。
         🔴 <strong>横スクロールは出ません。</strong></p>
      <p>👤 <strong>「担当ちがい」（v2.1.0）</strong>＝伝票の<strong>受付担当</strong>と PitFlow の<strong>フロント担当</strong>がちがう車です。
         <strong>フロント担当を伝票に合わせる</strong>ボタンで直せます（<strong>どなたでも</strong>）。
         🟢 <strong>売上の合計は動きません</strong>。動くのは<strong>フロント別の内訳</strong>だけです。
         ⚠ 伝票側の「専務」「チーフ」などの役職は、名前に読みかえてから入れます。
         ⚠ PitFlow のメンバーにいない名前は入れられません（そう出ます）。</p>
    ...
    shaken: `
      ...
      <p>車検の段取りを「行ける日候補 → 決定 → 完了／不合格 → 再検」で見える化する画面です。</p>
      <ul>
        <li><strong>行ける日候補</strong>（帯）：この表では<strong>見るだけ</strong>です（v2.54.0）。入れ替えは<strong>予約詳細の「車検スケジュール」</strong>から。
            表の<strong>一番左（お客様名＋車種）を押すと、その予約詳細が開きます</strong>。カーソルを乗せると、いつもの情報カードも出ます。</li>
        <li><strong>暫定予定（仮押さえ）</strong>（v2.54.0）：<strong>車 × 日のマスを押すと</strong>、上の決定カードと同じ形のカードがその場に置かれます。
            <br>・<strong>1台に1つだけ</strong>。別のマスを押すとそちらへ移ります。カードを押すと「この暫定を外す」が出ます。
            <br>🔴 <strong>まだ決定ではありません。</strong>MHSの当日ビューにも前日LINEの画像にも出ません。
            <strong>上の「決定」へドラッグすると本決まり</strong>になり、暫定は消えます。</li>
        ...
        <li>🔴🔴 <strong>決定チップを押すと出る窓は、線で3つの塊に分かれています</strong>（v2.57.0）。
            <br>・<strong>上</strong>＝いま決まっていることを直す（担当・陸運局・R →「この内容で保存」）
            <br>・<strong>まん中</strong>＝予定を動かす（午前⇄午後の変更／候補に戻す／カードを開く）
            <br>・<strong>下</strong>＝帰ってきた結果を入れる（下の3つ）
            <br>🔴 <strong>「午後に変更」を押すと、そのままラウンドを選ぶ窓に変わります</strong>。午前と午後でラウンドが変わるためです（1〜4Rぜんぶ出るので、例外の取り方もできます）。選ばずに閉じれば今のままです。
            <br>🔴 <strong>「予定を取り消す」は無くなりました</strong>。「↩ 候補（行ける日）に戻す」と実質同じだったためです（ゆうた指定）。</li>
        <li>🔴🔴 <strong>帰ってきた時の押し先は3つ</strong>（v2.56.0）。
            <br>・<strong>完了（一発合格）</strong>＝そのまま受かった。
            <br>・<strong>完了（再検合格）</strong>＝<strong>一度落ちたが、時間内にその場で直して、その回で受かった</strong>。実質1回なので<strong>台数は完了1台のまま</strong>ですが、「一度ひっかかった」ことが残ります。
            <br>・<strong>不合格（戻して修理）</strong>＝検査で駄目で<strong>自社に戻す</strong>。行く日は空になり、決め直しになります。
            <br>🔴 <strong>下の2つは、押すと「落ちた所」を書く窓に変わります</strong>（v2.57.0）。空のままでも記録できます。
            <strong>光軸・サイドスリップ・排ガス…</strong>などよく使う言葉は札で出るので、押すだけで入ります（もう一度押すと外れます）。
            書いた内容は<strong>予約詳細</strong>と、この画面の「不合格2」のような印にカーソルを乗せると読めます。</li>
        <li>🔴🔴 <strong>言葉の使い分け</strong>（v2.56.0・ゆうた確定）。同じ「再検」で終わった話とこれからの話が混ざらないように分けました。
            <br>・<strong>不合格</strong>（赤）＝<strong>行って落ちた日</strong>に付きます。v2.55.0 まで「再検」と出ていたのは全部これです。
            <br>・<strong>再検</strong>（オレンジ）＝<strong>不合格のあと、もう一度受験しに行く予定</strong>に付きます。
            決定カード・MHSの当日ビュー・前日LINEの画像に出るので、<strong>行く人が「持ち直しだ」と分かった状態で出られます</strong>。
            2回以上落ちている車だけ「再検2」のように回数が出ます。
            <br>・<strong>再検合格</strong>（緑）＝その回で受かった実績。決定カードでは枠が狭いので<strong>「再合」</strong>と出ます。</li>
        <li>🔀 <strong>再検の車は、押し先が2つだけ</strong>（v2.112.0・ゆうた指定）。
            <br>・1回目の車＝<strong>一発合格／再検合格（その場で直した）／不合格（持ち帰り）</strong>の3つ。
            <br>・一度でも不合格になった車（再検）＝<strong>今回は合格／今回も不合格</strong>の2つ。「一発合格」「再検合格」は出ません。
            <br>・再検の車は<strong>合格しても「再検」のまま</strong>です。決定カードは<strong>「再検済」</strong>、MHSの当日ボードと前日LINEの画像は頭が<strong>「再検」</strong>・右端が<strong>「済」</strong>になります（前は合格を押すと「車検」に戻っていました）。
            <br>・このボタンの並びは PitFlow の車検予定・予約詳細・MHS の当日ボードで同じです（1か所で決めています）。</li>
        <li>🔴 <strong>記録したあとでも直せます</strong>（v2.55.0）。予約詳細の不合格の記録は<strong>1本ずつ押せます</strong>。
            押すと、行った日・時間帯・担当・陸運局・R・理由を直す窓が出ます。
            <br>・<strong>「この記録を取り消す」</strong>＝押し間違えた時。記録を消して、<strong>その日を「決定」に戻します</strong>（担当も戻ります）。
            　⚠ もう別の日で決め直している時・済んでいる時は、<strong>行く日は動かさず記録だけ消します</strong>。
            <br>・MHSの当日ビューで不合格の行を押した時にも同じボタンが出ます。
            　⚠ 同じ日・同じ時間帯に記録が2つある時だけは、どちらか決められないので<strong>予約詳細から</strong>消してください。</li>
      </ul>
      <p>ヘッダーに「決定X／完了X（うち再合X）／不合格X／暫定X／候補X／未設定X」の件数が出ます。</p>
    ...
    shakenlog: `
      ...
      <ul>
        ...
        <li><strong>済</strong>（一発合格）／<strong>再合</strong>（再検合格＝その回で受かった）／<strong>不合</strong>（不合格＝自社に戻した）を色分けで表示。担当別の件数も分かります。
            ⚠ 再合は<strong>済の内訳</strong>です（別の台数ではありません）。</li>
      </ul>
    `,
    ...
    inspect: `
      ...
      <p>🗓 <strong>いちばん上は「月」です（v2.1.0）。</strong>‹ 2026年 8月 › で月を送ります。
         <strong>日常チェック</strong>では、選んだ月の車が<strong>先に</strong>出ます。
         🔴 <strong>ほかの月も下にぜんぶ出ます。1件も隠していません</strong>（「ほかの月の車 ◯件」を開けば見られます）。
         <strong>クォーターチェック</strong>では、その月の Q1〜Q4 を見ます。</p>
      <p>その下で<strong>2つを切り替えます。</strong></p>
      <ul>
        <li><strong>日常チェック</strong>＝PitFlow の中だけで分かる食い違いを、規則で拾います（お金はかかりません）。下の説明はこちらの話です。</li>
        <li><strong>クォーターチェック</strong>＝売上チェックリストPDF（整備ソフト）との突合と、AIチェック。およそ週に1度まとめて見るためのものです。
            PDFは<strong>ドラッグで放り込めます</strong>。結果は<strong>期間ごとに残る</strong>ので、月のはじめに Q1〜Q4 を1つずつ済ませていく使い方ができます（v1.184.0）。<br>
            🗓 <strong>期間は選ばなくて大丈夫です（v2.0.0）。</strong>PDFに書いてある「対象期間」を読んで、<strong>クォーターに自動で振り分けます</strong>。
            日付が少しズレて出ても、はみ出したぶんは<strong>別のクォーターとして分けて</strong>出します。月まるごとで出せば <strong>Q1〜Q4 に自動で振り分け</strong>ます。<br>
            🗓 <strong>切り替えるところは Q1〜Q4 の箱ひとつです（v2.2.0）。</strong>PDFを入れると、そのぶんが入っている箱が光り、
            <strong>このPDF ◯枚</strong>と<strong>残り〇件</strong>（または <strong>OK</strong>）が出ます。押すとその結果が開きます。<br>
            ⚠ そのクォーターの日がぜんぶ入っていない時は箱が<strong>点線</strong>になり、「<strong>3〜7日だけ</strong>」のように入っている日が出ます。
            <strong>「済」にはしません</strong>（まだ全部を見ていないので）。<br>
            🔴 <strong>「日付区分：売上日」で出したPDFだけ</strong>受け付けます。入金日などで出したものは日付の列が別物になるので、<strong>数字を1つも出さずに止まります</strong>。<br>
            🧹 <strong>別のPDFを入れ直す</strong>＝画面を空にします（残してある結果には触りません）。
            Q1〜Q4 の<strong>「×」</strong>＝その結果を消して「まだ」に戻します（練習ぶんの片づけ用）。
            <strong>「伝票を直した」の印は消しません。</strong><br>
            🔴 <strong>合計が合わない時は、数字を1つも出しません</strong>（読み取りに失敗したまま数字を見せないため）。</li>
      </ul>

      <h3><i data-ic=search data-ics=16></i> ③ 伝票の中身を見る（内容と原価・v2.3.0）</h3>
      <p>②で入れたPDFの<strong>中身そのもの</strong>（作業・部品・原価）を、
         <strong>うちの過去5か月（2026年3月〜7月・伝票864枚）</strong>と照らします。
         よその相場ではなく、<strong>うちがこれまで通してきた実績</strong>が物差しです。</p>
      <ul>
        <li>🔴 <strong>AIを走らせなくても、ここまでは出ます。</strong>お金はかかりません。
            AIは、そのあとで<strong>確度と見立てを付ける</strong>役です。</li>
        <li><strong>見る目線は5つ。</strong>
            <strong>掛け率がちがう</strong>（原価÷売価がふだんの幅から外れている）／
            <strong>売価がちがう</strong>（その品を、ふだんと違う値段で出している）／
            <strong>いつものが無い</strong>（オイル交換なのにドレーンワッシャーが入っていない等）／
            <strong>初めて見る品名</strong>／<strong>両方使う品名</strong>。</li>
        <li>掛け率は<strong>1本の線では切れません</strong>。産廃・ショートパーツは原価1円なのでほぼ0%、
            オイルやフィルターは50〜60%、Oリングやボルトなどの純正は90〜100%。
            <strong>3つの山</strong>に分かれています。だから品ごとに幅を持っています。</li>
        <li>ふだんから純正も社外も両方使っている品名（ドレーンワッシャーなど）は、
            <strong>外れ扱いにしません</strong>。毎回ひっかかってノイズになるので、
            「両方使う品名」に分けてあります。<strong>見たい時だけ開いてください。</strong></li>
        <li>🔴🔴 <strong>ここに出るのは「うちのふだんと違う」という事実だけです。</strong>
            まちがいとは言っていません。ラジエターひとつ取っても、純正しかない車種と社外がある車種があります。
            <strong>紙の伝票と照らして、決めるのは人です。</strong></li>
        <li>AIに渡すのは<strong>伝票の中身だけ</strong>です（電話番号・住所はそもそも載っていません）。
            <strong>「送るものを書き出す」</strong>で、何を送っているかいつでも自分で確かめられます。</li>
        <li>⚠ AIを走らせられるのは<strong>設定権限（管理）のある人だけ</strong>です（その都度お金がかかるため）。</li>
      </ul>

      <h3><i data-ic=printer data-ics=16></i> 元のPDFに刷り込んで出す（v2.4.0）</h3>
      <p>③のいちばん下の<strong>「元のPDFに刷り込んで出す」</strong>を押すと、
         入れた売上チェックリストPDFに<strong>色の帯と番号</strong>を刷り込み、
         最後に<strong>一覧のページ</strong>を足したものが出ます。
         <strong>紙の形はいつもと同じ</strong>なので、そのまま印刷して持ち歩けます。</p>
      <ul>
        <li>🔴 <strong>元の紙は1文字も変えません。</strong>上に重ねているだけです。</li>
        <li>紙に落とすのは<strong>まず見たい30件まで</strong>です（全部刷ると真っ赤で読めません）。
            何件のうち何件かは、一覧の頭に書いてあります。</li>
        <li>「いつものが無い」は<strong>そもそも紙に行がありません</strong>。帯は引けないので、
            一覧の後ろに「この行は紙にありません」としてまとめてあります。</li>
        <li>AIに見立てを付けてもらってから刷ると、<strong>AIのひとことも一覧に載ります</strong>。</li>
        <li>PDFは<strong>このパソコンの中だけ</strong>で読み書きします。どこにも送りません。</li>
      </ul>

      <h3><i data-ic=search data-ics=16></i> クォーターチェックの見かた（v2.2.0）</h3>
      <p>整備ソフトのことは、画面では<strong>「フロントマン」</strong>と呼びます（ソフトの名前です）。
         人の役職の「フロント」とは別の言葉なので、取り違えないでください。</p>
      <ul>
        <li>🔴🔴 <strong>フロントマンの伝票が正です。PitFlow をそこに合わせます。</strong>
            伝票を基にお金をいただいているので、基本はそちらが本当です（絶対ではありませんが、まずそう考えます）。</li>
        <li>🗂 <strong>入り口は4つだけです。</strong>
            <strong>データがちがう ／ 金額がちがう ／ 日付がちがう ／ OK</strong>。<br>
            🔴 <strong>1件は1か所にしか出ません</strong>（重いほうから順に入ります）。だから4つを足すと全部になり、
            <strong>お金の内訳もこの4つでそのまま差に戻ります</strong>（検算の「内訳＝差」がこの並びのまま生きています）。</li>
        <li>🔴🔴 <strong>いちばん重いのは「片方にしか売上が立っていない車」です。</strong>
            <strong>データがちがう</strong>の先頭に<strong>赤いカード</strong>で出ます。
            お金をもらったのに記録が無いか、記録があるのにお金をもらっていないかのどちらかです。<br>
            ・<strong>PitFlow にカードが無い</strong> → <strong>新規予約として作ってください</strong>（確認しただけでは消えません）<br>
            ・<strong>フロントマンに伝票が無い</strong> → <strong>伝票を立ててPDFを出し直す</strong>か、<strong>カードの実績を取り消す</strong>の2つだけです<br>
            ・<strong>カードは有るけれど、まだ返車済みにしていないだけ</strong> → <strong>黄色</strong>です。返車済みにすれば消えます。
            翌週返車にずれただけなら「確認した」で置いておけます。</li>
        <li>📅 <strong>日付は「売上日どうし」で見ます。</strong>
            前はフロントマンの売上日と PitFlow の<strong>実績日（返車日）</strong>をくらべていましたが、
            返車日は「車を返した日」なので意味がちがいます。<br>
            🔴 <strong>実績日は事実として出すだけ</strong>で、良し悪しは言いません。返車日はその日のうちに当日ビューから
            付けているので、だいたい合っています。答えは「<strong>このままでよい</strong>」だけです。<br>
            ⚠ 本当に返車済みを付けそこねた時は、<strong>その車のカードを開いて</strong>直してください。<br>
            ⚠ 売上日は 2026-08 に足したばかりなので、はじめのうちは「入っていません」がずらっと出ます。<strong>それが正しい姿</strong>です。</li>
        <li>🚗 <strong>車体番号（車台番号）</strong>を伝票から拾って、管理番号の右に出します。
            🔴 <strong>車体番号がそろっていれば、それが「同じ車」の答え</strong>です。
            車種の呼び方のちがい（「ＷＲＸ」と「スバル インプレッサ」など）は見ません。<br>
            PitFlow 側に番号が無いうちは車種で見ます。<strong>1回でも入庫すれば番号が入る</strong>ので、次からは番号で見られます。<br>
            番号がちがう車には「<strong>別の車かも</strong>」と赤く出ます。金額の話ではなく<strong>結びつけそのものを疑う</strong>合図なので、
            直すボタンは出しません。</li>
        <li>✍ <strong>全部片づいたら書き込めます（v2.2.0）。</strong>
            残りが <strong>0</strong> になると「<strong>書き込む</strong>」が出ます。押すと
            <strong>車の情報に車体番号</strong>、<strong>来店履歴にその時の伝票</strong>が入ります。<br>
            伝票の中身は、作業・部品・数量・単価・金額・<strong>原価</strong>・<strong>粗利</strong>まで全部です。
            車検の<strong>法定費用（自賠責・重量税・印紙代）</strong>は非課税なので、<strong>売上とも粗利とも分けて</strong>出します。<br>
            🔴 <strong>1つの予約に1つの伝票</strong>です。訂正されたら古いほうは消して置きかわります。<br>
            🔴 <strong>車体番号は上書きしません。</strong>すでに別の番号が入っていたら、書かずに知らせます
            （ナンバーの付け替えも、結びつけのまちがいも、どちらもありうるので人が見ます）。<br>
            ⚠ <strong>残りが0になるまで書けません。</strong>ズレたまま書くと、まちがった車にまちがった履歴がぶら下がります。</li>
        <li>👤 お客様の画面（<strong>顧客 → 車 → 来店履歴</strong>）で、その日の行を押すと<strong>伝票が開きます</strong>。
            車の情報には<strong>車体番号</strong>も出ます。</li>
      </ul>
      <ul>
        ...
        <li>🔢 <strong>1件ごとに番号が付いています（v1.178.0）。</strong>行の左端の <strong>F05-483102</strong> のような字です。<br>
            <strong>押すとコピー</strong>できます。「<strong>F05-483102 直しといて</strong>」と伝えれば、どの車のどの話かが1つに決まります。<br>
            🔴 <strong>この番号は、いつ・誰が見ても同じです。</strong>もう一度チェックを押しても、絞り込んでも、別のパソコンで見ても変わりません。
            直って消えたものが後でまた出た時も、<strong>同じ番号で戻ります</strong>。<br>
            ⚠ 頭の <strong>F05</strong> は<strong>規則の記号</strong>で、その規則の見出しの所にも同じ字が出ています。<br>
            ⚠ エラーが出た時の番号（<strong>PF-0412</strong> のような字）とは<strong>別物</strong>です。</li>
        ...
        <li>🔴🔴 <strong>規則は2種類あります（v1.173.0）。</strong><br>
            ...
            🔴 <strong>「確認した」は要判断の規則にしか出ません。</strong>抜け・矛盾には出しません。<br>
            ☎ <strong>「返車済みなのに、完TELを通っていない」（T02）だけは、ボタンが「完TELなしでOK」</strong>になっています（v2.93.0）。
            その場で待って帰ったなど、<strong>完TELをしないで返した車</strong>に押してください。
            確定金額と担当の抜けは<strong>別の規則が見ている</strong>ので、押しても隠れません。<br>
        ...
        <li>💴 <strong>「売上日と実績日で、月がちがう」（v1.185.0）</strong>＝<strong>伝票が立った月</strong>と
            <strong>PitFlow が売上に数えている月</strong>がちがう車です。そのままだと、整備ソフトの月次と PitFlow の月次が、その1台ぶんズレます。<br>
            🔴 <strong>クォーターがちがうだけなら言いません</strong>（週をまたいで返すのはふつうに起きるため）。<strong>月がちがう時だけ</strong>です。<br>
            直し方は「売上日」を直します（<strong>誰でも直せます・売上の数字は動きません</strong>）。<br>
            ⚠ v2.2.0 から、<strong>クォーターチェックの画面から実績日は直せません</strong>
            （返車日は当日に付けているので、直す場面がほぼ無いため）。本当に付けそこねた時は、その車のカードを開いて直してください。</li>
        ...
        <li><strong>書き出し</strong>＝結果を JSON で落とします。売上チェックリスト（整備ソフト）との突合や、AI にまとめて見てもらう時に使います。
            🔢 <strong>1件ごとの番号も一緒に入ります</strong>ので、前回の書き出しと同じ番号どうしで見くらべられます。</li>
      </ul>
    ...
    worksum: `
      <h2><i data-ic=user data-ics=16></i> 作業サマリー</h2>
      <p>一番上が<strong>「整備」</strong>と<strong>「車検ライン」</strong>の2つに分かれています（v2.58.0）。期間（当月／年度・月送り）は<strong>両方に効きます</strong>。</p>
      <h3>整備</h3>
      <p>返車済みのカードの点検担当者・整備担当者から、確定売上を「点検料→点検者／残り→整備者」に配分して、誰が何台・いくら作業したかを出します。<strong>中身は今までと同じです。</strong></p>
      <h3>車検ライン</h3>
      <p>その月に<strong>実際に陸運局へ行った記録だけ</strong>を集めた画面です。予定は1件も入りません。<strong>全体 → エラー → 詳細</strong>の3段。</p>
      <ul>
        <li><strong>全体</strong>＝ラインを通した回数（入場）／実台数／合格した台と内訳（一発・再検合格・戻して合格）／一発合格率／不合格。
            🔴 <strong>入場と台数は別もの</strong>です。1台が2回行けば入場は2回。<strong>その差が手戻り</strong>で、そのまま回送の時間と人の手を食っています。</li>
        <li><strong>エラー</strong>＝落ちた中身。「落ちた所」に書いた言葉を1語ずつ数えて多い順に出します（<strong>再検合格のぶんも数えます</strong>。ラインで一度ひっかかっているので、整備の材料としては同じです）。
            「戻し」＝そのまま不合格で持ち帰った数。<br>
            <strong>気をつけどころ</strong>は、その月の数字から言えることだけを出しています（多い順・差が大きい所）。数字が足りない時は出ません。</li>
        <li><strong>詳細</strong>＝回送の担当別／国産・輸入／陸運局・ラウンド・午前午後／不合格の明細。
            ⚠ 担当は<strong>陸運局へ行った人（回送）</strong>で、整備した人ではありません。通過率は腕の話ではなく、<strong>誰にどれだけ回送が乗っているか</strong>を見るためのものです。</li>
      </ul>
      <p>⚠ 数字が出るのは、帰ってきた時に<strong>「完了」か「不合格」を記録したぶん</strong>だけです。押していない車は台数に出ません。</p>
    `,
    ...
    settings: `
      ...
      <ul>
        ...
        <li><strong>作業タイプ（メニュー）は「見るだけ」です（v2.5.0）。</strong>ここからは増やせません・名前や色も変えられません。作業タイプは、諸費用の必須・車検予定への載り方・洗車やコーティングの段取り・データチェックの見張り・MHSへの出し方まで挙動が結びついているので、<strong>足したい／変えたいときは開発に言ってください</strong>（アプリ側に書いて配ります）。</li>
