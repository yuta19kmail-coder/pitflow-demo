...
(function (w) {
  ...
  var FIELDS = [
    ...
    /* 💴 v1.185.0 売上日＝整備ソフトの伝票が立った日。
       🔴🔴 v2.0.0（ゆうた指定 2026-08-23）**ここは「特例」。admin を付けない。**
       🗣「売上日も同様に基本はアーカイブはロック管理者のみ修正可能、
       　　**データチェックからはそこだけ修正できる特例権限**って扱いにして」
       ＝ 予約詳細カードでは、返車済み（アーカイブ）になったら**管理者だけ**
         （`sales-date.js` の `pitCanEditSalesDateOnCard`）。
         でも**この小窓からは誰でも直せる**。
       ◎なぜ特例にしてよいか
         ・ここは「**出ている数を 0 にする**」ための道。管理者待ちだと、指摘が出ているのに直す道が無い
         ・🟢 売上日は**売上の数字を1円も動かさない**（数える日は返車日のまま）
       ⚠ 確定金額・実績カウント日は**数字が動く**ので、この特例に**入れない**（下の admin:true のまま）。
