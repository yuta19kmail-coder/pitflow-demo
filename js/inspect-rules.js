...
(function (w) {
  ...
  var RULES = [
    ...
    { id:'F01', cat:'flow', level:'red',
      title:'返車予定日を過ぎたまま、まだ盤面にいる',
      why:'締めた過去の月を動かさないため、この車の金額は**今月に寄せて**数えています。'
         + '実際には終わっていないので、着地見込みがそのぶん膨らみます。',
      fix:'返車予定日を今の見込みに直すか、進んでいないなら理由をフローに残してください。',
      each: function(c, ctx){
        if (!isLive(c)) return '';
        var cd = countDate(c);
        if (!cd || cd >= ctx.today) return '';
        var n = days(cd, ctx.today);
        return '返車予定 ' + cd + '（' + n + '日前）／' + man(amountOf(c)) + ' を今月に寄せています';
      } },

    { id:'F02', cat:'flow', level:'amber',
      title:'同じエリアに長く止まっている',
      why:'工程が動いていません。お客様への連絡が止まっているか、札の移動を忘れています。',
      fix:'進んでいるならタスクボードで札を動かしてください。止まっているなら理由をフローに。',
      each: function(c, ctx){
        if (!isLive(c)) return '';
    ...
    { id:'F04', cat:'flow', level:'red',
      ...
      why:'実績カレンダーにも、どの月の売上にも数えられていません（静かに消えます）。',
    ...
    { id:'F08', cat:'flow', level:'red',
      ...
      why:'承認待ちは**わざと自動で未入庫に落としていません**（落とすと承認され忘れに気づけないため）。'
    ...
    { id:'F09', cat:'flow', level:'amber',
      ...
      why:'仮予約も**わざと自動で動かしていません**。本予約に変わっていないと、枠も代車も押さえられません。',
    ...
    { id:'D01', cat:'data', level:'red',
      ...
      why:'いまの保存の関門を通れない項目が空です。当日になってから慌てて聞くことになります。',
    ...
    { id:'D09', cat:'data', level:'gray',
      ...
      fix:'まとめて直すか、そのままでも大丈夫です。件数が多い時は「この規則は出さない」で黙らせられます。',
      each: function(c){
    ...
    { id:'D07', cat:'data', level:'amber',
      ...
      why:'顧客ページから来店履歴が引けません（1台ずつ手で探すことになります）。'
         + 'この車は**顧客控えに同じ電話番号（または同じお名前）の人がいる**ので、つなげられます。',
    ...
    { id:'T01', cat:'state', level:'red',
      ...
      why:'完TELを通った印が付いているのに、タスクは点検待ち・見積り中・連絡中のままです。'
         + '売上は「実績待」で数えているので、実際より確からしく見えています。',
      fix:'タスクボードで正しい列へ動かすか、返車の印を外してください。',
      each: function(c){
    ...
    { id:'T02', cat:'state', level:'red',
      ...
      why:'完TELの関門を通らずに実績になっています。確定売上も担当者も入らないまま固まっている可能性があります。',
    ...
    { id:'T06', cat:'state', level:'amber',
      title:'アーカイブ済みなのに、盤面の状態のまま',
      why:'片づいたはずなのに、状態は作業中のままです。あとで戻した時に行き先が分かりません。',
