...
(function (w) {
  ...
  var LIM = {
    ...
    /* 🔴 v2.82.0（ゆうた指定 2026-09-07）**小さい金額の倍率は見ない。**
       🗣「これは **20万円以下の倍率は考えなくていい**。例えば **20万が40万とかはチェック対象**」
       ＝ 5万が15万（3倍）は現場ではふつうに起きるが、金額としては小さいので追いかける意味が薄い。
  ...
  /* 🔴🔴 v1.173.0（ゆうた指定 2026-08-22）**規則は2種類ある。札もそれに合わせた。**
     ...
     | 種類 | 中身 | 終わり方 |
     |---|---|---|
     | **抜け・矛盾**（40本） | 金額が空・日付が空・状態が食い違う | **直すしかない。閉じる道は無い** |
     | 🆕 **要判断**（`judge:true`・**12本**） | 見積と確定が違う／けたが小さい／ずっと先 など | **見て「合っている」と確認したら閉じる** |
     ⚠ 本数は動く（v2.79.0 に L09、v2.82.0 に **R01・D01**、v2.93.0 に **T02** を要判断へ）。**足したら見張りの表も直す**
     🆕 v2.93.0 規則に `okLabel` / `okNote` を持たせると、**その規則だけ**「確認した」の言い方が変わる
        （T02＝「完TELなしでOK」）。⚠ 札の中身は同じ `'ok'`＝数の外し方も、古い端末の読み方も変わらない。
     　 ＝ `_見張り/test_inspect.mjs` の `JUDGE` の並び。
  ...
  var RULES = [
    ...
    { id:'M07', cat:'money', level:'amber', judge:true,
      ...
      each: function(c){
        ...
        if (q <= 0 || f <= 0) return '';
        /* 🔴 v2.82.0（ゆうた指定 2026-09-07）**大きいほうが 20万円以下なら見ない。**
           🗣「20万円以下の倍率は考えなくていい。例えば 20万が40万とかはチェック対象」
           ⚠ **小さいほう**で見ない（見積5万→確定40万 を落としてしまう）。**大きいほう**で見る。 */
        if (Math.max(q, f) <= LIM.gapMin) return '';
        if (f >= q * LIM.gapRate) return '見積 ' + man(q) + ' → 確定 ' + man(f) + '（' + (Math.round(f / q * 10) / 10) + '倍）';
        if (f * LIM.gapRate <= q) return '見積 ' + man(q) + ' → 確定 ' + man(f) + '（半分以下）';
        return '';
      } },

    /* 🔴🔴 v2.9.5 **M08「売上なしなのに金額が入っている」は出さないことにした**（ゆうた 2026-08-25）
       ----------------------------------------------------------------
       ◎決めごとどうしが正面衝突していた。
         card-view.js の `cvNoSaleArchive`（v1.99.0・ゆうた指定）は
         🔴「**金額は書き換えない。** 途中まで入っていた見積・受注の額は**本当に見積もった額**なので
            消さない。どこにも数えられないので残っていて害は無い」
         ＝ **金額が残っているのが正しい姿**。なのにこの規則は、その正しい姿を毎回 🟡 で出していた。
       ◎どちらを取るか
         金額を消す道は**データを捨てる**（見積もった事実が消える）。取れない。
         ＝ **規則のほうを引っ込める。**
       ⚠ 「混乱する」への手当ては、画面の**「売上なし」の札**が既にやっている
         （カード・来店履歴・アーカイブ帯に出る）。数字だけを見て誤解する道は無い。
       ⚠ 消さずに残す：また出したくなった時に、何を考えて止めたかが分かるように。
          **戻すなら、先に `cvNoSaleArchive` の決めごとを変えること。** 片方だけ戻さない。
    ...
    /* 🔴🔴 v2.22.0（2026-08-28・事故を受けて・ゆうた確定）
       予約 C63175 が**タスクボードから消えて未入庫にいた**（入庫の実績ごと消えていた）。
       自動処理（overdue-pit.js）は v2.22.0 から**入庫実績のある車を絶対に動かさない**ようにしたが、
       🔴 **動かさないだけで黙っていると、今度は「おかしいまま気づかれない」になる。**
       　 だからここに赤で出す。＝**止めたうえで、必ず人に見せる。** */
    /* ⚠ 番号は **F12**。F11 は 2026-08-25 に「出さない」と決めて**わざと消した番号**なので使わない
       （`test_inspect.mjs` の GONE 一覧が見張っている。実際にここで止められた）。 */
    { id:'F12', cat:'flow', level:'red',
      title:'入庫した実績があるのに、予約の状態に戻っている',
      why:'入庫した日（実入庫日）が残っているのに、カードは予約に戻っています。タスクボードから消えている状態です。端末どうしの行き違いで巻き戻った可能性があります。',
      fix:'タスクボードのどの列にいるべきか決めて、予約詳細から戻してください。本当に入庫を取り消したいなら「入庫を取り消して予約に戻す」を押してください（実入庫日も消えます）。',
      each: function(c, ctx){
        if (c.status !== 'reserved') return '';
        if (!t(c.actualInAt)) return '';
        if (c.archived) return '';
        return '実入庫日 ' + c.actualInAt + '（入庫日 ' + (t(c.reserveDate) || '未定') + '）';
      } },


    /* ── 予約 ───────────────────────────────────────────────── */
    /* ✅ v2.82.0（ゆうた指定 2026-09-07・R01-297688）**「これでいい」を押せるようにした。**
       🗣「これはあってるのよ実際には。だから **これでいい** が必要」
       ＝ 同じ車が同じ日に2枚あって**本当に正しい**ことがある（午前と午後で別の作業、など）。
       ⚠ 規則そのものは消していない＝**1件ずつ人が見て決める。**押した人と日付が残る。
       ⚠ 「確認した」は**その1件にだけ**効く。次に同じことが起きればまた出る。 */
    { id:'R01', cat:'resv', level:'red', judge:true,
      title:'同じ車が、同じ日に2枚',
    ...
    /* 🔗 v2.62.0（ゆうた指定 2026-09-05）**「まだ紐づいていない代車・自社車両」を0にする対象として出す。**
       ◎なぜ要るか
         紐づけは **車両管理で人が結んだ時だけ**できる（v2.62.0 でナンバーの自動引き当てをやめた）。
         ＝ 誰も結ばなければ **ずっと結ばれないまま、どこにも出ない。**
         🔴 だから件数を出す。**抜けは「許す」のではなく「0にする対象として数を出す」**
            （2026-09-04 の大前提・全アプリ共通）。
       ◎結ばれていないと何が起きるか
         ・顧客ビューにその車の印が出ない＝**うちの車だと分からない**
         ・代車を整備で入庫させても**お客様が付かない**（顧客側から整備の履歴が追えない）
       ⚠ 引退した車は数えない（もう使っていないので、結んでも意味が無い）。
       🔴 判定は `pitFleetUnlinked`（js/fleet-link.js）1本。ここで custId を見に行かない。 */
    { id:'L08', cat:'loaner', level:'amber',
      title:'代車・自社車両が、顧客控えの車と紐づいていない',
      why:'この車が顧客ビューで「代車」「自社車両」と分かりません。'
         + 'この車を整備で入庫させても、お客様が付かないので、顧客側から履歴が追えません。',
      fix:'車両管理 → その車を開く →「顧客車両との紐づけ」で相手を選んでください。'
         + '顧客控えにその車が無ければ、先に顧客・車両として登録します。',
      all: function(ctx){
        if (!w.pitFleetUnlinked) return [];
        return w.pitFleetUnlinked().map(function(x){
          var kind = w.pitFleetKindLabel ? w.pitFleetKindLabel(x.kind) : '代車';
          var name = t(x.v.name) || t(x.v.model) || x.v.id;
          var pl   = t(x.v.plate);
          /* 🔴 前の相手が消えている時と、はじめから結んでいない時は**言い分ける**
             （直し方が違う＝前者は選び直し、後者は初めて選ぶ） */
          var lost = !!(t(x.v.custId) || t(x.v.custVehId));
          return { kind:'veh', refId:x.v.id,
                   name: name + '（' + kind + (t(x.v.model) && t(x.v.model) !== name ? '・' + t(x.v.model) : '') + '）',
                   text: (lost ? '紐づけていたお客様の車が見つかりません（消された／まとめられた可能性）'
                               : 'お客様の車と紐づいていません')
                       + (pl ? '　ナンバー ' + pl : '　ナンバーが空です') };
        });
      } },

    /* 🚙🔴 v2.79.0（ゆうた指定 2026-09-07）**顧客控えの車種名に、代車の呼び名が入り込んでいる。**
       ◎なぜ要るか
         v2.78.0 まで、代車の整備カードを開いて閉じる／保存するたびに、
         **カードの車名（＝代車の呼び名）が、紐づけたお客様の車の「車種」に書き戻されていた。**
         ＝ 顧客ビューに「タント茶」「アクア1号」のような**社内の呼び名**が並んでしまう。
         🔴 v2.78.0 で道は塞いだが、**すでに書き換わったぶんは戻らない**
            （上書きした元の字はどこにも残っていない）。だから**数えて、人が直す。**
       ◎なぜ「見て決める」なのか
         もともと呼び名が車種名そのまま（呼び名も「タント」）のことがある。
         機械には見分けられないので、**合っているなら「確認した」で閉じてもらう。**
       🔴 判定は `pitFleetNameBled`（js/fleet-link.js）1本。ここで車種名を比べない。 */
    { id:'L09', cat:'loaner', level:'amber', judge:true,
      title:'顧客控えの車種名が、代車の呼び名になっている',
      why:'顧客ビューの車種名は車検証どおりの車種（タント・アクアなど）であるべきですが、'
         + '社内の呼び名（タント茶・アクア1号など）で上書きされている可能性があります。',
      fix:'顧客ビュー → その車 → 車種名を車検証どおりに直してください。'
         + 'もともとこの呼び名でよければ「確認した」を押すと数から外れます。',
      all: function(ctx){
        if (!w.pitFleetNameBled) return [];
        return w.pitFleetNameBled().map(function(x){
          var kind = w.pitFleetKindLabel ? w.pitFleetKindLabel(x.kind) : '代車';
          var who  = t(x.cust.name) || t(x.cust.kana) || '(無名)';
          var pl   = t(x.veh.plate);
          return { kind:'veh', refId:x.v.id,
                   name: (t(x.v.name) || t(x.v.model) || x.v.id) + '（' + kind + '）',
                   text: who + ' 様の車の車種名が「' + x.呼び名 + '」＝この' + kind + 'の呼び名と同じです'
                       + (pl ? '　ナンバー ' + pl : '') };
        });
      } },
    ...
    /* ✅ v2.82.0（ゆうた指定 2026-09-07・D01-774597）**「これでいい」を押せるようにした。**
       🗣「これも これでいい」
       ＝ 決まりができる前のカードで、**もう埋めようがない**（聞く相手がいない・その項目が要らない車）
         ことがある。⚠ 規則は消していない＝**1件ずつ人が見て決める。**押した人と日付が残る。
       ⚠ ここは赤（抜け）のままにしてある。**「見て決めるのが仕事」の所に手段を置いただけ**で、
         「やらなくていい」にはしていない。 */
    { id:'D01', cat:'data', level:'red', judge:true,
    ...
    /* ☎🔴🔴 v2.93.0（ゆうた指定 2026-09-13・番号 T02-245425）**「完TELなしでOK」で閉じられるようにした。**
       🗣「T02-245425 これ、直した以外にも **完TELなしでOK** のチェックがほしい」
       ◎なぜ閉じてよいか
         完TELをしないで返す車は**ふつうに居る**（その場で待って帰った など）。
         そういう車は**直しようがない**＝「抜け・矛盾」のままだと、**永久に0にならない。**
       🔴 **お金と担当の抜けは、これで閉じても逃げない。**
          確定金額は **M02**、整備・点検担当は **T03**、フロント担当は **T04** が**別に見ている**
          （callDone の説明と同じ）。T02 を閉じるのは「完TELをしなかった」ことだけ。
       ⚠ 札の中身は他の要判断と同じ `'ok'`。言い方だけ `okLabel` で変えている
          ＝ 数の外し方・押した人と日付・古い版の端末の読み方（v2.83.0）が**そのまま効く**。 */
    { id:'T02', cat:'state', level:'red', judge:true,
      okLabel:'完TELなしでOK',
      okNote:'完TELをしないで返した車（その場で待って帰った など）。数から外れます。確定金額と担当は別の規則が見ています',
      ...
      fix:'カードを開いて確定金額と担当を入れてください。'
        ...
        + '完TELをしないで返した車なら「完TELなしでOK」を押してください。',
    ...
    /* ✅ v2.73.0（ゆうた指定 2026-09-05）**チェック担当もこの1本で数える。**
       🔴 ここは1文字も足していない。物差しの `pitMechUnsettled` に役が1つ増えただけ。
          ＝ 条件を書き写していないので、こういう時に**直す場所が1か所で済む**。
       ⚠ チェック担当は**導入日（mech-pick.js の CHECKER_FROM）より前のカードでは数えない**
          （昔のカードには入れようが無く、要対応が数千件出て**本当に直すべき抜けが埋もれる**ため）。
          出す・出さないの線引きも `pitMechUnsettled` の中。ここでは見ない。 */
    { id:'T03', cat:'state', level:'red',
      title:'作業が終わっているのに、担当が入っていない',
      why:'メカニックの実績（作業サマリー）に数えられません。誰がやったか・誰が確かめたかが残りません。'
  ...
  function sweepEscapes(){
    ...
    /* 🔴🔴🔴 v2.83.0（ゆうた報告 2026-09-07）**「確認した」の札を消しに行くのをやめた。**
       -----------------------------------------------------------------
       🗣「R01-297688 とか D01-774597 が、**確認したでチェックしてても数秒で戻ってきちゃう**」
       ◎正体＝**古い版の端末が消していた。**
         ここは前まで「要判断でない規則に付いた『確認した』は外す」を**その端末が知っている規則表**で
         判断して、**消して保存**していた。
         v2.82.0 で R01・D01 を要判断にしたので、**まだ新しくなっていない端末では
         R01・D01 は「要判断ではない」**＝押したそばから消され、クラウド越しに全員の画面から消えていた。
         ＝ **押した本人の画面は正しく、もう1台が消していた。**1台の台では絶対に捕まらない。
       🔴🔴 **これは CoreBoard「動かしたはずのカードが戻る」（2026-09-06）とまったく同じ家族。**
         決めごともそのまま＝**自分の版の知識だけで、ほかの版が書いたものを消しに行かない。**
         ⚠ 規則を1本足すたびに、古い端末が新しい札を消す道ができてしまう
           （知らない規則IDも `judgeOf` では「要判断でない」になるため）。
       ◎かわりにどうするか＝**読む側で無視する。**（下の `push()`）
         書いてあるものは消さず、**要判断でない規則の「確認した」は数から外さない**だけにした。
         ＝ 抜け道にはならず、版が違っても**データは減らない。**
       ⚠ **`pitInspectMark` の書き込み側の関門は前から在る**（要判断でない規則には付けられない）。
         つまり、そもそも新しく付くことはない。ここで消す必要がもう無い。 */
  ...
  function pitInspectRun(opt){
    ...
    function push(rule, ref, text, kind, name){
      ...
      var m = mk[key];
      /* 🔴 v2.83.0 **要判断でない規則に付いている「確認した」は、読む時に無視する。**
         ＝ 抜け道にならない。**そして札は消さない**（消すと、版のちがう端末どうしで消し合う）。
         ⚠ 「直した」の札はどの規則にも付くので、ここでは触らない。 */
      if (m && m.v === 'ok' && !rule.judge) m = null;
      findings.push({
        ...
        okLabel: rule.okLabel || '', okNote: rule.okNote || '',   /* 🆕 v2.93.0 「確認した」の、その規則での言い方 */
