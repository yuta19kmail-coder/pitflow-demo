...
(function () {
  ...
  function monthBar(){
    ...
    return '<div class="ins-month">'
         ...
         +     (UI.mode === 'quarter'
                ? 'この月の Q1〜Q4 を突き合わせます'
                : (!UI.mode
                   ? 'どちらかを押すと、<b>いまのデータで考えてから</b>出します'
  ...
  /* ================================================================
     🚪🔴🔴 v2.96.0（ゆうた指定 2026-09-13）**開いた瞬間は、ボタン2つだけ。押した時に、いまのデータで考える。**
     ----------------------------------------------------------------
     ...
     ◎正体（4件 → 押すと0件）
       Qの枠の「残 ◯件」は、**前にそのQを突き合わせた時に保存した件数**だった（`x.run.直す件数`）。
       押すと残してある伝票を**いまのデータ**で組み直すので、その間に直したぶんが消えて0になる。
       ＝ 枠の数字が**古いまま**出ていた。押す前と後で数字が変わる＝信用できない。
     ◎いま
       ① **ビューに入った時**（別のビューから来た時だけ）… `pitInspectReset` で全部捨てる。
          出すのは月のバーと、日常／クォーターの**ボタン2つだけ**。数字は1つも出さない。
       ② **日常チェック**を押す … その場で全カードを見る（もともと毎回いまのデータで数えている）
       ③ **クォーターチェック**を押す … その月の Q1〜Q4 を、残してある伝票と**いまのデータで全部組み直してから**出す
          （`pitQFreshMonth`）。枠の件数は**組み直した生の数**。残りがあるQを自動で開く。
       ④ ビューの中で行き来する … 組み直した結果を**持ったまま**切り替える（読み直さない）
       ⑤ ビューを出て戻る … ①へ（また組み直す）
     🔴 「入った時」の見分けは **views.js の `_pitSame`**（同じビューの描き直しか）。
        背後の描き直し（同期が来た時の `showView(state.currentView)`）では捨てない。
     ⚠ 組み直すのは「**この月の結果をまだ何も持っていない時**」だけ。PDFを読んでいる途中・読んだあとは触らない
        （触ると、いま読んだPDFの結果を保存の結果で上書きしてしまう）。
     ================================================================ */
  window.pitInspectReset = function () {
    UI.mode = '';
    UI.res = null;
    UI.level = ''; UI.cat = '';
    if (window.pitQClearForMonth) window.pitQClearForMonth(ym());
    if (UI.q){ UI.q.list = null; UI.q.listBusy = false; }
  };
  ...
  function _renderInspect(body) {
    ...
    /* 🚪 v2.96.0 まだどちらも押していない＝ボタン2つだけ（数字を1つも出さない） */
    if (!UI.mode){
      body.innerHTML = monthBar() + modeBar()
        + '<div class="ins-start">押した時に、<b>いまのデータで考えて</b>から結果を出します。'
        + '<span>このビューを出ると、次に開いた時はまた最初から考え直します。</span></div>';
      if (window.pitIcons) try { pitIcons(body); } catch(e){}
      return;
    }
    if (UI.mode === 'quarter'){
      /* 🚪 v2.96.0 この月の結果をまだ何も持っていなければ、いまのデータで組み直す（中身は quarter.js の1本） */
      var _q = UI.q;
      if (window.pitQFreshMonth && (!_q || (_q.groups == null && !_q.res && !_q.saved && !_q.busy && !_q.pdf && !_q.err))){
        window.pitQFreshMonth(ym());
      }
      body.innerHTML = monthBar() + modeBar() + quarterHtml(); if (window.pitIcons) try { pitIcons(body); } catch(e){} return;
    }
