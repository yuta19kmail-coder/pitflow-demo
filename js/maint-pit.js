...
(function (w) {
  ...
  /* ==================================================================
     ...
     ⚠ お客様欄は入庫の時に、**車両管理で人が結んだ相手**を写す（`_intakeGo`）。
        ここでは自社の名前で置いておく＝**空のカードを作らない**（v1.56.1 の教訓）。
        （v2.62.0 でナンバーの自動引き当ては廃止。`_ownerFill` はもう無い＝2026-09-07 に古い書き置きを直した）
     🚙🔴 **車名の欄は「代車の呼び名」**（車両管理で入れた車種名。例＝タント茶・アクア1号）。
        ＝ 車検証の車種名ではないので、**顧客控えに書き戻してはいけない。**
          v2.78.0 で `upsertCustomerFromCard`（customers.js）の入口を塞いだ。詳しくはあちらの説明。
     ================================================================== */
