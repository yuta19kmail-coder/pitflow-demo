...
(function () {
  ...
  /* ==================================================================
     📦🔴🔴 v2.94.0（ゆうた指定 2026-09-13・番号 T03-605881 / T03-802675）
     **物販のカードは、3つの役とも「なし」と読む。**
     ------------------------------------------------------------------
     ...
     ◎正体
       物販（v2.51.0）は作業者の欄を**どこにも出していない**（カード詳細・工程の窓・作業完了の窓）。
       なのに「決まっているか」の物差し（ここ）は物販を知らず、**入れる所が無い欄を「未入力」と言っていた**
       ＝ データチェック T03 が、直しようのない物販を数えていた。
     🔴 **カードに「なし」の印は書かない。読む時に「なし」とみなす。**
       ・書くと、あとで物販を外して普通の作業にした時に**「なし」だけが残る**
         ＝ 本当に担当を入れ忘れても T03 が黙る（いちばん困る壊れ方）。
       ・読む側なら、物販を外した瞬間に**ふつうの物差しへ戻る**。いま引っかかっている2枚もデータを触らずに消える。
     ⚠ **人が入っている役は、人のほうを優先**（物販でも名前が入っていれば、その名前を出す）。
     ⚠ 物販かどうかの物差しは **pit-share.js の `pitCardGoods` 1本**。ここで workType を見ない。
     ================================================================== */
  function goodsNone(c, role){
    if (!(c && window.pitCardGoods && window.pitCardGoods(c))) return false;
    var a = Array.isArray(c[role]) ? c[role].filter(Boolean) : [];
    return a.length === 0;
  }
  function isNone(c, role){ return !!(c && (c[noneKey(role)] || goodsNone(c, role))); }
