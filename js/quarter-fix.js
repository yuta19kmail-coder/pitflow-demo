...
(function (w) {
  ...
  function fixKinds(p){
    ...
    /* ② 実績日は**もう出さない**（ゆうた 2026-08-24）
       ----------------------------------------------------------------
       🗣「実績日は返車日だから、常に当日ビューから返車済みにするわけで、ほぼズレない。
          だから実績日を変更は基本存在しないでいいと思う。本当にすごくイレギュラーの時のみ」
       🔴 だから**この画面からは直せない**。本当に付けそこねた時は、その車のカードを開いて直す。
       ⚠ 代わりに「このままでよい」の印だけ置ける（下の `keepKinds`）。 */
    ...
    /* ④ 金額（🔴 売上の金額が動く・管理者だけ） */
    /* ⚠ v2.2.0 ±1円も出す。**グループ（quarter-match.js）の数え方と食い違わせない**
       ＝ 金額がちがうの欄に出ているのに直すボタンが無い、という形を作らない。 */
    if (num(p.soft.金額) !== num(p.pit.確定金額)){
      out.push({
        kind: '金額',
        now: yen(p.pit.確定金額) + '円',
        to: num(p.soft.金額),
        label: '確定金額を ' + yen(p.soft.金額) + '円 にする',
        why: '伝票と ' + (p.差 > 0 ? '+' : '') + yen(num(p.soft.金額) - num(p.pit.確定金額)) + '円 ちがいます',
        can: canFinal(),
        重い: true
      });
    }
  ...
  /* ================================================================
     🗓 v2.2.0 「このままでよい」＝実績日の答えは**これ1つだけ**
     ----------------------------------------------------------------
     🔴 返車日は当日ビューから付けている＝合っている。伝票の日とちがっても直さない。
        でも**見たという印**は残せないと、残りがいつまでも0にならない。
     ⚠ 印では合計・差・内訳・検算を1ミリも動かさない（あちらは PDF が言っている事実）。
     ================================================================ */
  function keepKinds(p){
    var out = [];
    if (p && p.期間の外) {
      out.push({ kind: '実績日', label: 'このままでよい', 保つ: true,
        why: '返車日は当日に付けているので、伝票と日がちがっても直す必要はありません' });
    }
    /* 🚗 v2.9.3（ゆうた 2026-08-25「OKボタンが既に押されているのかな？ そしてその状態で残っている」）
       ----------------------------------------------------------------
       v2.9.2 で「**ナンバーが読めない**＝要注意」を「データがちがう」に入れたが、
       **押せる札を1つも用意しなかった**ので、その行は永久に残り続けた。
       🔴 **人に見てもらう行には、必ず「見た」と言える札を付ける。**
          そうしないと「どうしたらいい？」の行き止まりになる。
       ⚠ ナンバーが無いままなので、直す先は PitFlow 側にも伝票側にも無い。
          だから「直す」ではなく「**確かめた**」＝印を置くだけ。 */
    if (p && p.同一性 === 'plateNG') {
      out.push({ kind: '同じ車', label: '確かめた（同じ車です）', 保つ: true,
        why: 'ナンバーが読めないので、機械では同じ車だと言い切れません。目で見て確かめたら押してください' });
    }
    return out;
  }
  ...
  /* ================================================================
     🔴🔴 v2.2.0 片方にしか無い行の印
     ----------------------------------------------------------------
     ⚠ 置けるのは「**カードは有る／まだ返車済みにしていないだけ**」の行だけ。
        ・PitFlow にカードが無い → 作るまで消さない
        ・フロントマンに伝票が無い → 立てるか取り消すまで消さない
        ＝ 伝票が無いままだと、このあとの伝票の履歴も車体番号も残せない（ゆうた 2026-08-24）。
     ================================================================ */
  function oneKey(no){ return 'ONE|' + t(no); }
  function oneMarkOf(x){
    if (!x || !x.カード) return null;                    /* カードが無い行には置けない */
  ...
  function rowLeft(p){
    ...
    var a = fixKinds(p).concat(keepKinds(p));
    return a.filter(function (k) { return !markOf(k.kind, p.soft, id); }).length;
  }

  /* 🔴 v2.9.3 その行に**もともと仕事があったか**。
     ＝「押したから0」と「そもそも0」を、画面が見分けるために要る。
       これが無いと、直すところが無い行まで**押したような灰色**になる（ゆうたが踏んだ）。 */
  function rowTotal(p){
