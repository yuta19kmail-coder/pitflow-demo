...
(function (w) {
  ...
  /* ================================================================
     ...
     ================================================================ */
  /* 🧾🧾 v2.9.1 **返車が先・伝票があと**（ゆうた 2026-08-25・QP-415514）
     ----------------------------------------------------------------
     🗣「板金に近いが、**返車が先で伝票があと**のパターン。今後は無くすようにするが、
     　　実際リアルパターンとして少数ある。**これはQまたぎと同じような扱いにしてほしい**」

     ◎実データ（V48797・有限会社 小松園芸・キャンター・82,470円）
       PitFlow の実績日 **8/03（Q1）** ／ 伝票 **8/20（Q3）・0685**
       ＝ 車は返して実績も立っている。**伝票だけ17日あとに切った。**

     🔴 v2.8.3 の `crossOnly` は「**売上日どうしがぴったり**」を要る条件にしていたので、
        このパターン（カードに売上日が入っていない・伝票の日を知らない）は拾えなかった。
     🔴 だから **前後関係**で見る。`実績日 < 伝票の売上日` ＝「返車が先・伝票があと」。
        ⚠ カードに売上日が**入っている**なら、そちらは確かめられるので**今までどおり厳しく**見る
          （入っているのに伝票とちがう＝直す先がある）。
     ⚠ 月またぎは**ここでも除く**（`crossOnly` の頭で切っている）。
        実績が8月・伝票が9月なら月次の数字がズレる＝2026-08-08 の決めごとどおり 🔴 全件NG。 */
  function slipLater(pair){
    if (!pair.同じ車 || !pair.金額一致) return false;
    var cd = t(pair.pit && pair.pit.数える日), sd = t(pair.soft && pair.soft.売上日);
    if (!cd || !sd) return false;
    if (!(cd < sd)) return false;                                  /* 実績が先・伝票があと だけ */
    var ps = t(pair.pit && pair.pit.売上日);
    if (ps && ps !== sd) return false;                             /* 売上日が入っていて食い違う＝直す先がある */
    return true;
  }
  ...
  function crossOnly(pair){
    ...
    if (pair.売上日差 && pair.売上日差.kind === 'same') return true;    /* 売上日どうしがぴったり */
    /* 🧾 v2.9.1 返車が先・伝票があと（上の `slipLater` を見よ） */
    return slipLater(pair);
  }
  /* お知らせの言い方。⚠ 画面はこれを読むだけ（自分で言い換えない）。 */
  function crossWhy(pair){
    if (!crossOnly(pair)) return '';
    if (pair.売上日差 && pair.売上日差.kind === 'same'){
      return '伝票を切った日と車を返した日でQがまたがっただけです。金額・車・売上日は合っています';
    }
    return '車を返したあとに伝票を切っています（実績 ' + t(pair.pit.数える日)
         + ' → 伝票 ' + t(pair.soft.売上日) + '）。金額と車は合っています';
  }
  ...
  function match(soft, pit, opt){
    ...
    var softRows = (soft || []).map(function (r, i) {
      return {
        ...
        読んだ: t(r.読んだ),                    /* ⏳ v2.104.0 この伝票をいつ読んだか（「伝票を直した」の印を確かめる） */
    ...
    function take(sr, pr, how){
      ...
      _p.お知らせ理由 = crossWhy(_p);          /* 🧾 v2.9.1 言い方は物差しが決める */
    }
  ...
  w.pitQCrossWhy  = crossWhy;     /* 🧾 v2.9.1 お知らせの言い方（Qまたぎ／返車が先・伝票があと） */
