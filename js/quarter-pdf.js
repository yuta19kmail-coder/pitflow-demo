...
(function (w) {
  ...
  function detRow(line, det){
    ...
    line.cells.forEach(function (c) {
      ...
      /* 🗒 v2.12.3 その字が**名前の列に入っているか**。
         ＝ 続きの紙に刷り直される伝票の頭（「システム」「車台番号」など）を
            コメントと取りちがえないための物差し（実物で 543件 巻き込んだ）。 */
      if (nmc && c.x >= nmc.x - 8 && c.x <= nmc.r + 8) 名前列 = true;
      name.push({ x: c.x, w: c.w || 0, s: txt });
    });
    /* 名前の頭に付く作業コード（330・7501…）と、行末に落ちた数字を落とす */
    /* 🔴 v2.12.4 **離れている字の間には空白を入れる**（linesOf と同じ物差し）。
       ＝ `E90 WMWMM32030TL25287` が `E90WMWMM32030TL25287` にくっつかないように。 */
    var nmRaw = '', prevEnd = null;
    name.forEach(function (c) {
      if (prevEnd != null && (c.x - prevEnd) > 1.5) nmRaw += ' ';
      nmRaw += c.s; prevEnd = c.x + c.w;
    });
    nmRaw = nmRaw.replace(/\s+/g, ' ').trim();
    /* ⚠ 頭の作業コード（330・7501…）と、行末に落ちた数字を落とすのは**ふつうの明細だけ**。
       コメントには掛けない（車台番号の後ろ半分が消える。実物で踏んだ）。 */
    var nm = nmRaw.replace(/^[0-9]+\s*(?=[^0-9])/, '')
                  .replace(/[\s0-9.]+$/, '');
    return { 名: nm, 名素: nmRaw, f: f, 名前列: 名前列 };
  }
  function dnum(f, k){
  ...
  function parse(lines){
    ...
    lines.forEach(function (ln) {
      ...
      else if (/一般消費税/.test(x)){
        var cm = moneys(x); if (cm.length) cur.消費税 += cm[cm.length - 1];
        /* 🗒 ここから下は**締めの欄**。この先に出てくる「名前だけの行」は明細のコメントではない
              （実物では `一般消費税 → 預り金 → 伝票計` の並びで、預り金は締めの見出し。109枚で17件） */
        cur._締め = true;
      }
      ...
      /* 🧾 v2.2.0 明細の1行。⚠ 見出し（【一般整備】など）も**そのまま残す**
        ...
        if (hd){
          ...
          if (rw){
            ...
            /* ================================================================
               🗒🗒 v2.12.3 **フロントマンの「コメント」行**（ゆうた 2026-08-25）
               ----------------------------------------------------------------
               🗣「これはフロントマン上ではコメントという扱いで、**前後に文脈がある**」
               🗣「例えばこの場合なら、これが**点検に掛かってる**感じ」
               🗣「だからこのコメントに関しては**全面的に入れこんでほしい**」
               ◎実物（8/1〜8/23・109枚）で58件。PDF の上では
                 **名前だけがあって、数量・単価・金額・原価・区分・担当者名称が1つも無い列**。
                 ＝ お金にならないので、ここで `amt == null` になり**まるごと消えていた**。
               例）高橋様・MINI（伝票071）
                   【一般整備】→ 燃料圧力測定 → エンジンタイミング測定
                              → テスター診断・エラーチェック・実 点検 16,500
               🔴 **掛かる先はこちらで推さない。並んでいた順のまま入れる**（ゆうた指定）。
                  実物では「見出しの直後＝次の作業に掛かる」「作業の直後＝前の作業に掛かる」
                  「締めの手前＝伝票ぜんぶの注意書き」の3通りが混ざっている。
                  推せば必ずどれかを取りちがえる。紙と同じ並びなら、読む人が読めば分かる。
               ⚠ 金額は 0。だから **`明細が合う` の検算には1円も影響しない**。
               ⚠ PDF 側で**全角16文字で切れている**（「作業後のエラー消去・学習値リセッ」）。
                  続きは紙にも刷られていないので、こちらでは足せない。
               ================================================================ */
            /* ⚠ **明細が1つ出てから**でないと拾わない。伝票の頭のほう（車台番号・「整備」など）にも
                  「名前だけの列」があるので、そこを巻き込む（実際に踏んだ）。
                  実物は必ず 見出し【…】 が先に来るので、これで頭とは混ざらない。 */
            /* 🔴🔴 コメントの字は**そのまま**入れる（`rw.名素`）。
               ⚠ ふつうの明細に使っている `rw.名` は、頭の作業コードと**行末の数字**を落としている。
                  それをコメントに掛けると、**車台番号の後ろ半分が消える**。
                  実物：`E90 WMWMM32030TL25287`（p1.pdf・伝票0129）が `E90WMWMM32030TL` になっていた。
               🗣 ゆうた「まれに**伝票のコメントとして**入れるケースがあるよ」
                  ＝ 車体番号がコメントに書いてあるのは**正しい伝票**。はじいてはいけない
                    （車両が仮登録で車台番号の欄が空の時に、ここに打ってある）。 */
            else if (rw.名素 && rw.名前列
                     && !Object.keys(rw.f).length && !cur._締め && cur.明細.length){
              cur.明細.push({ 種: 'コメント', 名: rw.名素, 区分: '',
                              数量: 0, 単価: 0, 金額: 0, 原価: 0, ことわり: '',
                              頁: ln.頁 || 0, 枠: ln.枠 || null });
            }
          }
        }
      }
