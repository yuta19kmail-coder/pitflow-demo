...
(function (w) {
  ...
  function slimPitOnly(r){
    return {
      ...
      /* 🟢 v2.101.0 どのQのどの伝票で照合できたか（「Q3 で照合済み（Q-######）」と言うため） */
      別のQ先: r.別のQ先 ? { label: s(r.別のQ先.label), from: s(r.別のQ先.from), 売上日: s(r.別のQ先.売上日), 伝票: s(r.別のQ先.伝票) } : null
    };
  }
  /* ================================================================
  ...
  function slimSoft(r){
    return {
      ...
      車台: s(r.車台 || r.車体番号),
      読んだ: s(r.読んだ),   /* ⏳ v2.104.0 いつ読んだ伝票か（開き直しても「伝票を直した」の印を確かめられる） */
