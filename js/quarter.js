...
(function (w) {
  ...
  function nokoriOf(R){
    var n = 0;
    /* 🔢🔴 v2.100.0（ゆうた報告 2026-09-13「8月Q4が修正箇所1になってる。でもデータ違う・金額違う・日付違うのどれにもない」）
       **箱から「チェック済み」へ移した行は、残りにも数えない。**
       ◎正体 … 箱（splitDone）は `pitQRowDone`＝「直した記録（DID）がある」だけで片づいた扱いにして箱から出す。
         ところがここは `pitQRowLeft > 0`（押していない印が残っているか）**だけ**で数えていた。
         ＝ 実際に直したが「このままでよい」をまだ押していない行が、**どの箱にも居ないのに残り1件**になる。
       🔴 物差しは箱と同じ `pitQRowDone` 1本。片づいた行はここでも数えない。 */
    (R.結びついた || []).forEach(function (p) {
      if (p.組 === 'ok') return;
      if (w.pitQRowDone && w.pitQRowDone(p)) return;
      if (!w.pitQRowLeft || w.pitQRowLeft(p) > 0) n++;
    });
