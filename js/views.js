...
function showView(viewId){
  ...
  /* 🚪 v2.96.0 データチェックに**別のビューから入った時だけ**、前の結果を捨ててボタン2つから（背後の描き直しでは捨てない） */
  if (viewId === 'inspect' && !_pitSame && window.pitInspectReset) pitInspectReset();
...
/* ===================================================================
   ...
   ◎ゆうたの言葉
     ...
     🗣「予約の段階から預かりって表記始まってない？」← そのとおりだった。
   ◎正体＝**どの画面も起点に `reserveDate`（入庫“予定”日）を使っていた。**
     ＝ ① 予定日に来なかった車が、来ていないのに「3日目」と増えていく
     　 ② 予定より遅れて入った車の日数が、遅れたぶんだけ水増しされる
     　 ③ ダッシュボードの「預かり中◯台」に、まだ来ていない予約が混ざる
   🔴 **決めごとは1つだけ＝「入庫していないものは預かりではない」。**
     ・起点は **実入庫日（`actualInAt`）**。無い昔のカードは入庫日で数える（今までどおり）
     ・まだ入庫していないカードは **台数にも日数にも出さない**
   ⚠ **予想の画面だけは別。** ダッシュボードの「2週間先の混み具合」と「最短入庫日」は
